...
use imports::imports;
use osg_ship_api::abi::{self as w, Record};
use osg_ships::*;
use std::{
    collections::HashMap,
    mem::size_of,
...
mod computer;
pub use computer::*;
pub use osg_model::drawing as screens;
use screens::*;
...
pub trait ScanSource: Send + Sync {
    ...
    fn query_output_bytes(
        &self,
        _query: &osg_model::ProgramQuery,
        _display: bool,
        _capacity: osg_model::wasm_world::ReplyCapacity,
        maximum: usize,
    ) -> Result<usize> {
        Ok(maximum)
    }
    ...
    fn query(
        ...
        _query: osg_model::ProgramQuery,
        _display: bool,
        _reply_capacity: osg_model::wasm_world::ReplyCapacity,
    ) -> Result<osg_model::ProgramReply> {
...
impl Controller {
    ...
    pub fn execution_status(&self) -> osg_model::ExecutionStatus {
        if self.waiting_for_gas {
            osg_model::ExecutionStatus::WaitingForGas
        } else if self.is_suspended() {
            osg_model::ExecutionStatus::Suspended
        } else {
            osg_model::ExecutionStatus::Ready
        }
    }
