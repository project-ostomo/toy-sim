...
use toy_sim_ships::{DeviceCommand, DeviceStatus};
            ...
            .map(|(index, source)| {
                if let DeviceSource::MicropulseGenerator(part) = source {
                    ...
                    let exhaust_velocity =
                        toy_sim_ships::STANDARD_GRAVITY_M_S2 * specific_impulse_s;
                ...
                match design.parts[*part].definition.equipment {
                    ...
                    Equipment::Shield {
                        ...
                        radiator_area_m2,
                        feed_rate_kg_s,
                        emissivity: toy_sim_ships::thermal::EMISSIVITY,
...
pub fn query_work(query: &toy_sim_model::ProgramQuery) -> u64 {
    ...
    match query {
        ProgramQuery::Orrery { .. } => toy_sim_model::local_space::QUERY_GAS,
