...
impl<T> LuminosityBvh<T> {
    /// Allocated node storage, excluding allocations owned by payloads.
    pub fn allocated_bytes(&self) -> usize {
        self.nodes.capacity() * std::mem::size_of::<Node<T>>()
    }

    /// Visit bounds and leaves, pruning when the visitor returns false.
    /// Children nearer the query origin are visited first. Errors stop traversal.
    pub fn try_visit<E>(
        &self,
        origin: Position,
        ...
        while let Some(index) = stack.pop() {
            ...
            if !visit(node.bounds, node.max_luminosity, leaf)? {
                continue;
            }
            if let NodeKind::Branch { left, right } = node.kind {
                let a = self.nodes[left].bounds.distance_squared(origin);
                let b = self.nodes[right].bounds.distance_squared(origin);
    ...
    /// Builds a balanced median BVH. See [`Self::build_median`] for construction details.
    ///
    /// Returns leaf indices in input order. Empty input produces an empty tree.
    /// Each leaf contains one object; nodes use depth-first preorder storage.
    ///
    /// # Panics
    ///
    /// Panics on invalid bounds or non-finite or negative luminosity.
    pub fn build(entries: impl IntoIterator<Item = BvhEntry<T>>) -> (Self, Vec<usize>) {
        Self::build_median(entries)
    }
    ...
    pub fn build_median(entries: impl IntoIterator<Item = BvhEntry<T>>) -> (Self, Vec<usize>) {
        build(entries)
    }

    /// Builds a Morton LBVH using 21 bits per axis and a stable radix sort.
    ///
    /// This has the same input and output contract as [`Self::build_median`], but
    /// splits on Morton prefixes rather than spatial medians. Equal codes
    /// retain input order and split at the median. Bounds remain exact;
    /// quantization affects topology only. Nodes retain depth-first preorder.
    ///
    /// # Panics
    ///
    /// Panics on invalid bounds or non-finite or negative luminosity.
    pub fn build_morton(entries: impl IntoIterator<Item = BvhEntry<T>>) -> (Self, Vec<usize>) {
        morton::build(entries)
    }
}
