...
use toy_sim_model::{GalacticPosition, Pose, Provenance, ownership::*};
...
impl Fixture {
    fn new(observed: bool) -> Self {
        ...
        let mut runtime = toy_sim_ship_wasm::ControllerRuntime::default();
        let ship = world
            .spawn((
                ShipDesign(design),
...
fn interception_uses_observed_collision_course_without_hidden_missile_target() {
    ...
    track
        .tags
        .insert(toy_sim_model::Tag::Kind("missile".into()));
    ...
    for pending_lease_request in [false, true] {
            ...
            .last_request;
        {
            ...
            if pending_lease_request {
                software.inbox.push(toy_sim_ship_wasm::Request {
                    id: lease_request,
