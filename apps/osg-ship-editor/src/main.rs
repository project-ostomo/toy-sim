...
mod viewport;
use bevy::prelude::*;
use osg_ship_wasm::ControllerRuntime;
use osg_ships::*;
use osg_ui::bevy_egui::{EguiGlobalSettings, EguiPrimaryContextPass};
...
pub struct Editor {
    ...
    pub descriptions: Vec<osg_ui::parts::PartDescription>,
    pub catalogue_search: String,
    pub category: osg_ui::parts::Category,
    pub inspector: ui::InspectorTab,
    pub undo: Vec<ShipBlueprint>,
    pub redo: Vec<ShipBlueprint>,
    pub selected: Option<u64>,
    pub place: Option<String>,
    pub placement_tanks: Vec<osg_ships::Tank>,
    pub orientation: u8,
    pub revision: u64,
    pub dirty: bool,
    pub file: String,
...
impl Default for Editor {
    fn default() -> Self {
        ...
        let descriptions = catalogue
            .parts
            .iter()
            .map(|part| osg_ui::parts::PartDescription::new(part, &catalogue))
            .collect();
        Self {
            ...
            devices_mode: false,
            preview_thrust: 1.,
            viewport: osg_ui::egui::Rect::NOTHING,
...
impl Editor {
    ...
    pub fn launch(&mut self) -> anyhow::Result<()> {
        ...
        self.runtime
            .validate_program(self.ship.controller_bytes())?;
        let dir = std::env::temp_dir().join("osg-ship-editor");
        ...
        anyhow::ensure!(
            binary.is_file(),
            "Build osg-debug and osg-server first, or set its executable path in the inspector"
        );
        self.child = Some(
            ...
            DefaultPlugins
                ...
                .set(WindowPlugin {
                    primary_window: Some(Window {
                        title: "OpenSpaceGame Ship Editor".into(),
...
mod tests {
    ...
    fn assembly_deletion_and_undo_restore_self_contained_design() {
        ...
        assert!(e.validation.is_ok(), "{:?}", e.validation);
        let path =
            std::env::temp_dir().join(format!("osg-ship-roundtrip-{}.ship", std::process::id()));
