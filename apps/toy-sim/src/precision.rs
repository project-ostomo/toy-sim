mod interpolation;
pub use interpolation::{InterpolatedTransform, PresentationPose, PresentationVelocity};
...
/// The common clock for interpolated hulls, sensor estimates, and spatial publications.
pub fn presentation_time(time: &bevy::prelude::Time<bevy::prelude::Fixed>) -> f64 {
    (time.elapsed_secs_f64() - time.delta_secs_f64()
        + time.overstep_fraction_f64().clamp(0., 1.) * time.delta_secs_f64())
    .max(0.)
}
