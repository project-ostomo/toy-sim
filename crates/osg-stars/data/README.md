...
This directory holds the star catalogue that `osg-stars` embeds with `include_bytes!`, plus the files that describe how it was produced. The format and tools are documented in [docs/gaia-catalogue.md](../../../docs/gaia-catalogue.md).
...
python3 tools/import_gaia.py crates/osg-stars/data/gaia-dr3-earth-million.stars \
    crates/osg-stars/data/gaia-dr3-earth-million.csv
```
...
Rebuild after replacing the `.stars` file, because it is embedded in every binary that depends on `osg-stars`. The test `embedded_million_star_catalogue_is_queryable_without_a_runtime_file` expects exactly 1,000,000 stars and between 6,000 and 7,000 stars brighter than magnitude 6 from the origin. Update that test if you embed a different selection.
