...
pub fn tick(world: &mut World, duration_ms: f64) {
    ...
    if !slow
        && !bevy::log::tracing::enabled!(target: "toy_sim_server::timing", bevy::log::tracing::Level::DEBUG)
    {
        return;
    }
    ...
    macro_rules! report {
        ($level:ident) => {
            bevy::log::$level!(target: "toy_sim_server::timing",
...
pub fn publication(tick: u64, duration_ms: f64, display_ms: f64, sessions: usize) {
    if duration_ms >= 100.0 {
        bevy::log::warn!(target: "toy_sim_server::timing", tick, duration_ms, display_ms, sessions,
            "slow server display and snapshot publication");
    } else {
        bevy::log::debug!(target: "toy_sim_server::timing", tick, duration_ms, display_ms, sessions,
