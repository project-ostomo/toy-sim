...
fn transcript(client: &[u8; 66], server: &[u8; 66]) -> [u8; 32] {
    let mut hash = blake3::Hasher::new_derive_key("toy-sim transport v1 handshake");
    hash.update(client);
    hash.update(server);
...
fn derive(
    ...
    Ok((
        blake3::derive_key("toy-sim transport v1 c2s", &material),
        blake3::derive_key("toy-sim transport v1 s2c", &material),
    ))
}

fn account_challenge(account: AccountId, transcript: &[u8; 32]) -> [u8; 32] {
    let mut hash = blake3::Hasher::new_derive_key("toy-sim transport v1 account authentication");
...
pub async fn server<S: AsyncRead + AsyncWrite + Unpin>(
    ...
    let account = toy_sim_model::Id(auth[..16].try_into()?);
