...
use osg_ship_view::PartVisualAssets;
use osg_ships::appearance::PreparedAppearance;
...
fn sync_ships(
    ...
    for (view_entity, observation, camera, render_camera, projection, camera_transform) in &views {
        ...
        let private_view = owned
            ...
            .is_some_and(|(_, ship, _, _)| ship.0.presence != osg_model::travel::Presence::Space);
...
fn spawn_ship(
    ...
    loader: &AssetServer,
    thermal: &osg_ship_view::thermal::ThermalAssets,
    transform: Transform,
) -> Entity {
    let entity = commands.spawn((transform, Visibility::default())).id();
    osg_ship_view::spawn_parts(commands, entity, design, assets, loader);
    if let Some(shield_radius) = design.shield_radius {
        commands.spawn((
            ChildOf(entity),
            Shield,
            osg_ship_view::thermal::ThermalSphere {
                temperature_k: 0.,
                strength: 0.,
            },
...
fn apply_visuals(
    ...
    mut plumes: Query<(
        &ChildOf,
        &mut osg_ship_view::plume::EnginePlume,
        Option<&osg_ship_view::plume::RcsNozzle>,
    )>,
    mut shields: Query<(&ChildOf, &mut osg_ship_view::thermal::ThermalSphere), With<Shield>>,
    mut barrels: Query<(
        &osg_ship_view::weapon::WeaponVisual,
        &ChildOf,
        &mut Transform,
    )>,
) {
...
fn mechanism_time(
    ...
    mut clock: ResMut<osg_ship_view::mechanisms::MechanismTime>,
) {
