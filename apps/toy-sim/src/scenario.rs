//! Temporary startup state, kept in code until a gamesave format exists.
use bevy::math::{DQuat, DVec3};
use rand::{RngExt, SeedableRng};
use rand_chacha::ChaCha8Rng;

/// Spin and throttle used only by the maneuvering-target regression fixtures.
#[cfg(test)]
pub const TRAFFIC_TUMBLE_BODY: DVec3 = DVec3::new(0.17, 0.23, 0.11);
#[cfg(test)]
pub const TRAFFIC_CHALLENGE_THROTTLE: f64 = 0.1;

#[cfg(test)]
pub use barrage::*;

#[cfg(test)]
mod barrage {
    use super::*;

    pub const INITIAL_SLUG_COUNT: usize = 100;
    pub const INITIAL_SLUG_SPEED: f64 = 100_000.0;
    pub const INITIAL_SLUG_FINAL_SPEED: f64 = 350_000.0;
    pub const INITIAL_SLUG_MASS: f64 = 0.01;
    pub const INITIAL_SLUG_RADIUS: f64 = 0.005;

    pub fn slug_arrival_tick(index: usize) -> usize {
        let fraction = (index / 2) as f64 / (INITIAL_SLUG_COUNT / 2 - 1) as f64;
        ((10.0 + 20.0 * fraction) * crate::simulation::TICK_RATE_HZ).round() as usize
    }

    fn slug_speed(index: usize) -> f64 {
        let fraction = (index / 2) as f64 / (INITIAL_SLUG_COUNT / 2 - 1) as f64;
        INITIAL_SLUG_SPEED + fraction * (INITIAL_SLUG_FINAL_SPEED - INITIAL_SLUG_SPEED)
    }

    /// Fifty opposing pairs, increasing in energy to overwhelm a stock ship near
    /// the middle of the barrage. Reverse the same
    /// kick/drift used by physics, so orbital curvature does not spoil the aim.
    pub fn incoming_slugs(
        universe: &crate::orrery::Universe,
        epoch: hifitime::Epoch,
        player: crate::precision::GalacticPosition,
        velocity: DVec3,
        outward: DVec3,
    ) -> Vec<(crate::precision::GalacticPosition, DVec3)> {
        let dt = 1.0 / crate::simulation::TICK_RATE_HZ;
        let last_tick = (30.0 / dt).round() as usize;
        let sources: Vec<_> = universe
            .iter()
            .filter(|body| universe.gravity_applies(&body.name, player))
            .collect();
        let gravity: Vec<Vec<_>> = (0..last_tick)
            .map(|tick| {
                let time = epoch + hifitime::Duration::from_seconds(tick as f64 * dt);
                sources
                    .iter()
                    .map(|body| {
                        (
                            universe
                                .solve_position(&body.name, time)
                                .unwrap()
                                .relative_to(player),
                            crate::physics::GRAVITATIONAL_CONSTANT * body.mass,
                        )
                    })
                    .collect()
            })
            .collect();

        let acceleration = |position: DVec3, sources: &[(DVec3, f64)]| {
            sources
                .iter()
                .map(|&(source, mu)| {
                    let offset = source - position;
                    offset * (mu / offset.length().powi(3))
                })
                .sum::<DVec3>()
        };
        let mut position = DVec3::ZERO;
        let mut velocity = velocity;
        let mut coasting = vec![(position, velocity)];
        for sources in &gravity {
            velocity += acceleration(position, sources) * dt;
            position += velocity * dt;
            coasting.push((position, velocity));
        }

        (0..INITIAL_SLUG_COUNT)
            .map(|index| {
                let tick = slug_arrival_tick(index);
                let (mut position, mut velocity) = coasting[tick];
                // Opposing pairs limit accumulated recoil before the shield fails.
                let direction = if index % 2 == 0 { 1.0 } else { -1.0 };
                velocity -= outward.normalize() * (direction * slug_speed(index));
                for sources in gravity[..tick].iter().rev() {
                    position -= velocity * dt;
                    velocity -= acceleration(position, sources) * dt;
                }
                (player.offset_by(position), velocity)
            })
            .collect()
    }
}

pub const INITIAL_SCENARIO: InitialScenario = InitialScenario {
    body: "Helion I Neris",
    altitude: 40_000_000.0, // High orbit, outside the five moons.
    orbit_seed: 42,
    camera_distance: 100.0,
    camera_yaw: 0.0,
    camera_pitch: 0.0,
    traffic_count: 1,
    // Preserve the former nearest target (Traffic 243) from the 500-ship fleet.
    traffic_orbit_start: 243,
};

/// Initial circular orbits, expressed in inertial axes relative to the chosen body.
#[derive(Clone, Debug)]
pub struct InitialScenario {
    pub body: &'static str,
    pub altitude: f64,
    pub orbit_seed: u64,
    pub camera_distance: f64,
    pub camera_yaw: f64,
    pub camera_pitch: f64,
    pub traffic_count: usize,
    /// First traffic orbit in the seeded sequence; index zero belongs to the explorer.
    pub traffic_orbit_start: usize,
}

impl InitialScenario {
    pub fn nearest_traffic(states: &[(DVec3, DVec3)]) -> Option<usize> {
        let player = states.first()?.0;
        states
            .iter()
            .enumerate()
            .skip(1)
            .min_by(|(_, a), (_, b)| {
                a.0.distance_squared(player)
                    .total_cmp(&b.0.distance_squared(player))
            })
            .map(|(index, _)| index)
    }

    pub fn validate(&self, universe: &crate::orrery::Universe) -> anyhow::Result<()> {
        let body = universe
            .get_body(self.body)
            .ok_or_else(|| anyhow::anyhow!("unknown starting body {}", self.body))?;
        self.relative_state(body.radius, body.mass)?;
        let atmosphere_height = body.atmosphere.as_ref().map_or(0.0, |a| a.height);
        anyhow::ensure!(
            self.altitude > atmosphere_height,
            "starting orbit must be above the atmosphere"
        );
        anyhow::ensure!(
            self.camera_distance.is_finite()
                && self.camera_distance > 0.0
                && self.camera_yaw.is_finite()
                && self.camera_pitch.is_finite(),
            "invalid initial camera"
        );
        Ok(())
    }

    /// Explorer first, then the selected stretch of the reproducible orbital sequence.
    pub fn fleet_states(&self, radius: f64, mass: f64) -> anyhow::Result<Vec<(DVec3, DVec3)>> {
        let (r, speed) = self.orbit_dimensions(radius, mass)?;
        anyhow::ensure!(
            self.traffic_orbit_start > 0,
            "traffic cannot reuse the explorer's orbit"
        );
        let mut rng = ChaCha8Rng::seed_from_u64(self.orbit_seed);
        let mut states = vec![sample_orbit(r, speed, &mut rng)];
        for _ in 1..self.traffic_orbit_start {
            sample_orbit(r, speed, &mut rng);
        }
        states.extend((0..self.traffic_count).map(|_| sample_orbit(r, speed, &mut rng)));
        Ok(states)
    }

    pub fn relative_state(&self, radius: f64, mass: f64) -> anyhow::Result<(DVec3, DVec3)> {
        let (r, speed) = self.orbit_dimensions(radius, mass)?;
        Ok(sample_orbit(
            r,
            speed,
            &mut ChaCha8Rng::seed_from_u64(self.orbit_seed),
        ))
    }

    fn orbit_dimensions(&self, radius: f64, mass: f64) -> anyhow::Result<(f64, f64)> {
        let r = radius + self.altitude;
        anyhow::ensure!(
            self.altitude > 0.0 && r.is_finite() && r > 0.0 && mass.is_finite() && mass > 0.0,
            "invalid starting orbit"
        );
        Ok((
            r,
            (crate::physics::GRAVITATIONAL_CONSTANT * mass / r).sqrt(),
        ))
    }
}

fn sample_orbit(radius: f64, speed: f64, rng: &mut ChaCha8Rng) -> (DVec3, DVec3) {
    // Uniform unit quaternion: randomize both the plane and phase without
    // concentrating orbital normals near the poles (as uniform inclination would).
    let u = rng.random::<f64>();
    let a = rng.random::<f64>() * std::f64::consts::TAU;
    let b = rng.random::<f64>() * std::f64::consts::TAU;
    let rotation = DQuat::from_xyzw(
        (1.0 - u).sqrt() * a.sin(),
        (1.0 - u).sqrt() * a.cos(),
        u.sqrt() * b.sin(),
        u.sqrt() * b.cos(),
    );
    (
        rotation * (DVec3::Z * radius),
        rotation * (DVec3::X * speed),
    )
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn incoming_slugs_intercept_the_coasting_player_between_ten_and_thirty_seconds() {
        let universe = crate::orrery::Universe::init(crate::orrery::example_config()).unwrap();
        let body = universe.get_body(INITIAL_SCENARIO.body).unwrap();
        let epoch = hifitime::Epoch::from_mjd_utc(0.0);
        let (position, velocity) = INITIAL_SCENARIO
            .relative_state(body.radius, body.mass)
            .unwrap();
        let mut player = universe
            .solve_position(INITIAL_SCENARIO.body, epoch)
            .unwrap()
            .offset_by(position);
        let mut velocity = universe
            .solve_velocity(INITIAL_SCENARIO.body, epoch)
            .unwrap()
            + velocity;
        let mut slugs = incoming_slugs(&universe, epoch, player, velocity, position);
        assert_eq!(slugs.len(), 100);
        assert!((slugs[0].0.relative_to(player).length() / INITIAL_SLUG_SPEED - 10.0).abs() < 0.01);
        assert!(
            (slugs[99].0.relative_to(player).length() / INITIAL_SLUG_FINAL_SPEED - 30.0).abs()
                < 0.01
        );

        let dt = 1.0 / crate::simulation::TICK_RATE_HZ;
        let mut intercepted = 0;
        for tick in 0..300 {
            let time = epoch + hifitime::Duration::from_seconds(tick as f64 * dt);
            let sources: Vec<_> = universe
                .iter()
                .map(|body| (body, universe.solve_position(&body.name, time).unwrap()))
                .collect();
            let advance = |position: &mut crate::precision::GalacticPosition,
                           velocity: &mut DVec3| {
                for &(body, source) in &sources {
                    if universe.gravity_applies(&body.name, *position) {
                        let offset = source.relative_to(*position);
                        *velocity += offset.normalize()
                            * (crate::physics::GRAVITATIONAL_CONSTANT * body.mass
                                / offset.length_squared())
                            * dt;
                    }
                }
                *position = position.offset_by(*velocity * dt);
            };
            advance(&mut player, &mut velocity);
            for (index, (position, velocity)) in slugs.iter_mut().enumerate() {
                let arrival = slug_arrival_tick(index);
                if tick < arrival {
                    advance(position, velocity);
                }
                if tick + 1 == arrival {
                    assert!(
                        position.relative_to(player).length() < 0.01,
                        "slug {index} missed at tick {arrival}"
                    );
                    intercepted += 1;
                }
            }
        }
        assert_eq!(intercepted, 100);
    }

    #[test]
    fn fleet_has_reproducible_high_circular_orbits_with_varied_planes() {
        let orrery = crate::orrery::Universe::init(crate::orrery::example_config()).unwrap();
        let scenario = &InitialScenario {
            traffic_count: 500,
            traffic_orbit_start: 1,
            ..INITIAL_SCENARIO.clone()
        };
        scenario.validate(&orrery).unwrap();
        let body = orrery.get_body(scenario.body).unwrap();
        assert_eq!(scenario.altitude, 40_000_000.0);
        let states = scenario.fleet_states(body.radius, body.mass).unwrap();
        assert_eq!(states.len(), 501);
        let challenger = InitialScenario::nearest_traffic(&states).unwrap();
        assert!(challenger > 0);
        let range = states[challenger].0.distance(states[0].0);
        assert!(
            states
                .iter()
                .skip(1)
                .all(|s| s.0.distance(states[0].0) >= range)
        );
        assert!(InitialScenario::nearest_traffic(&states[..1]).is_none());
        assert!(InitialScenario::nearest_traffic(&[]).is_none());
        assert_eq!(
            states,
            scenario.fleet_states(body.radius, body.mass).unwrap()
        );
        assert_eq!(
            states[0],
            scenario.relative_state(body.radius, body.mass).unwrap()
        );
        let mut normal_sum = DVec3::ZERO;
        let mut position_sum = DVec3::ZERO;
        let mut prograde = 0;
        for &(position, velocity) in &states {
            assert!((position.length() - body.radius - scenario.altitude).abs() < 1e-6);
            assert!(position.normalize().dot(velocity.normalize()).abs() < 1e-12);
            let circular = crate::physics::GRAVITATIONAL_CONSTANT * body.mass / position.length();
            assert!((velocity.length_squared() / circular - 1.0).abs() < 1e-12);
            let normal = position.cross(velocity).normalize();
            normal_sum += normal;
            position_sum += position.normalize();
            prograde += usize::from(normal.y > 0.0);
        }
        // Catch accidentally shared planes/phases or one-sided orbit sampling.
        assert!((200..300).contains(&prograde));
        assert!(normal_sum.length() / (states.len() as f64) < 0.1);
        assert!(position_sum.length() / (states.len() as f64) < 0.1);
    }

    #[test]
    fn default_has_only_the_explorer_and_the_former_nearest_target() {
        let universe = crate::orrery::Universe::init(crate::orrery::example_config()).unwrap();
        let body = universe.get_body(INITIAL_SCENARIO.body).unwrap();
        let old = InitialScenario {
            traffic_count: 500,
            traffic_orbit_start: 1,
            ..INITIAL_SCENARIO.clone()
        }
        .fleet_states(body.radius, body.mass)
        .unwrap();
        let current = INITIAL_SCENARIO
            .fleet_states(body.radius, body.mass)
            .unwrap();
        assert_eq!(current.len(), 2);
        assert_eq!(current[0], old[0]);
        assert_eq!(
            current[1],
            old[InitialScenario::nearest_traffic(&old).unwrap()]
        );
    }
}
