...
pub fn advertised_principal(iff: Option<&IffIdentity>) -> Option<Principal> {
    iff.filter(|iff| iff.enabled)
        .map(|iff| Principal::Player(iff.owner))
}

pub fn color(standing: Option<Standing>) -> egui::Color32 {
    match standing {
...
pub fn label(standing: Option<Standing>) -> &'static str {
    match standing {
...
pub fn symbol(standing: Option<Standing>) -> &'static str {
    match standing {
...
mod tests {
    ...
    fn colors_follow_advertised_identity_and_keep_unknown_distinct() {
        ...
        let mut directory = SocietyPresentation::default();
        directory.viewer = observer;
        directory
            .advertised
            .insert((Some(advertised), None), Standing::Hostile);
        let iff = IffIdentity {
            owner: advertised,
            faction: None,
            labels: BTreeSet::new(),
