...
fn track<'a>(world: &'a World, parent: Entity, target: ContactRef) -> Option<&'a osg_model::Track> {
...
pub fn launch(
    ...
    let clearance =
        osg_ships::thermal::shield_radius(parent_design.radius) + missile_design.radius + 1.;
...
pub fn reconcile(world: &mut World) {
    ...
    let parents: Vec<_> = world
        ...
        .iter(world)
        .filter(|(_, _, presence, _, _)| presence.0 == osg_model::travel::Presence::Destroyed)
