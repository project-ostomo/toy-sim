...
use toy_sim_stars::{GalacticPosition as P, Star, StarCatalogue, StarId, VisibilityQuery};
...
fn python_converter_fixture_matches_portable_loader() {
    ...
    assert!(
        (catalogue.stars()[0].position.relative_to(P::ZERO).length() / toy_sim_stars::PARSEC - 10.)
            .abs()
            < 1e-12
    );
