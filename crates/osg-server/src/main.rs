...
async fn main() -> Result<()> {
    ...
    let path = args.next().context(
        "usage: osg-server <config.toml> [--ready-file PATH] [--shutdown-on-stdin-close]",
    )?;
    if path == "--init" {
        let directory = args.next().context("--init requires a directory")?;
        anyhow::ensure!(args.next().is_none(), "unexpected argument");
        return osg_server::provision::demo(std::path::Path::new(&directory));
    }
    let mut options = osg_server::launch::Options::default();
    ...
    osg_server::launch::run(std::path::Path::new(&path), options).await
}
