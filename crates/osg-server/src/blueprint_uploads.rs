...
use osg_model::{AccountId, industry::BlueprintUploadAck};
use std::{
...
impl BlueprintUploads {
    ...
    pub(crate) async fn receive<R: AsyncRead + Unpin>(&self, read: &mut R) -> Result<[u8; 32]> {
        ...
        loop {
            if bytes.len() == osg_ships::MAX_FILE {
                let mut extra = [0];
                ensure!(
    ...
    tokio::time::timeout(timeout, async {
        let ack = match uploads.receive(stream).await {
            ...
            Err(error) => {
                ...
                let mut end = reason
                    .len()
                    .min(osg_model::industry::MAX_BLUEPRINT_UPLOAD_ERROR_BYTES);
        ...
        let bytes = osg_protocol::encode_blueprint_upload_ack(&ack)?;
...
mod tests {
    ...
    async fn read_ack(stream: &mut DuplexStream) -> BlueprintUploadAck {
        ...
        osg_protocol::decode_blueprint_upload_ack(&bytes).unwrap()
    }
    ...
    async fn invalid_uploads_return_bounded_rejections_and_release_all_bytes() {
        ...
        truncated.pop();
        let oversized = wire(&vec![7; osg_ships::MAX_FILE + 1]);
