...
fn main() -> anyhow::Result<()> {
    ...
    let stars = toy_sim_universe::civilization::stars();
    ...
    for (index, system) in map.systems.iter().enumerate() {
        ...
        let orrery = star.map(|star| toy_sim_universe::generation::system(star, &system.name));

        let stellar: Vec<serde_json::Value> = orrery
            .iter()
            .flat_map(|orrery| orrery.bodies.iter())
            .filter_map(|body| body.stellar.as_ref().map(|stellar| {
                serde_json::json!({
                    "name": body.name,
        ...
        let planets: Vec<serde_json::Value> = orrery
            ...
            .flat_map(|orrery| orrery.bodies.iter())
            .filter(|body| {
                matches!(body.class_params, toy_sim_universe::orrery_cfg::BodyClass::Planet)
            })
            .map(|body| {
