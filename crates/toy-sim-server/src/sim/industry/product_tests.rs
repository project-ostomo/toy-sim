...
use toy_sim_model::ownership::{AccessGrant, AccessPolicy};
use toy_sim_ships::DeviceSetting;
...
impl Fixture {
    fn new() -> Self {
        let account = Id::new();
        let mut blueprint = toy_sim_ships::ntr_patrol();
    ...
    fn dock(&mut self) {
        ...
        let distance = world.get::<vessel::ShipDesign>(station).unwrap().0.radius
            ...
            + toy_sim_model::travel::DOCKING_CLEARANCE_M * 0.5;
        let velocity = world
