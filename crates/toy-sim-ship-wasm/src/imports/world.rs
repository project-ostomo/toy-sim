use super::*;
use toy_sim_model::{ProgramQuery, ProgramReply, wasm_world::ReplyCapacity};
use toy_sim_ship_api::world as a;
...
pub(super) fn register(linker: &mut Linker<Host>) -> Result<()> {
    ...
    metered!(
        ...
        |mut caller: Caller<'_, Host>, pointer: u32, output: u32, capacity: u32, header: u32| {
            ...
            let limit = (capacity as usize).min(toy_sim_model::local_space::MAX_LOCAL_OBSTACLES);
            prepare(
                &caller,
                query,
                ReplyCapacity {
    ...
    metered!(
        ...
        |mut caller: Caller<'_, Host>,
         ...
         fuels: u32,
         fuel_capacity: u32| {
            if count as usize > toy_sim_model::routing::MAX_ORDERS {
                return Err(w::ERR_LIMIT.into());
            }
            ...
            let actions: Result<Vec<_>, _> = (0..count)
                .map(|index| {
                    ...
                    toy_sim_model::travel::Order::try_from(&record)
                        .map_err(|_| CallError::from(w::ERR_ARGUMENT))
                })
                .collect();
            let request = toy_sim_model::routing::Request {
                id: record.id,
            ...
            metered!(
                ...
                },
                {
                    status((|| {
                        ...
                        let action = toy_sim_model::ProgramAction::try_from(&record)
                            .map_err(|_| w::ERR_ARGUMENT)?;
...
fn route_plan(
    ...
    let order_limit = (capacity as usize).min(toy_sim_model::routing::MAX_ORDERS);
...
fn write_route(
    ...
    let (value, actions, resources) = toy_sim_model::wasm_world::route_records(*id, status);
    if let ProgramReply::Route {
        status: toy_sim_model::routing::Status::Ready { plan },
        ..
    } = &reply
    {
