...
pub use dynamic_bvh::{DynamicLuminosityBvh, DynamicTreeStats, InvalidLeafHandle, LeafHandle};
...
impl Aabb {
    ...
    pub fn distance_squared(self, from: Position) -> f64 {
        (0..3)
            .map(|axis| {
...
impl Aabb {
    pub fn segment_entry(self, start: Position, end: Position, radius_m: f64) -> Option<f64> {
        service::query::segment_entry(self, start, end, radius_m)
    }
}
