...
- **Explorer.** "Orbital explorer" starts in a circular orbit 40,000 km above Helion I Neris, on the day side, with the tangential direction selected from seed 42 ([scenario.rs](../crates/toy-sim-server/src/sim/scenario.rs)). Its design is the configured `ship` blueprint, or the armed starter. If the blueprint fails to load or compile, the server logs "Cannot load ship" and spawns no ships at all, so scenario setup fails.
- **Traffic.** "Traffic 001" uses the armed starter, starts 100 km from the explorer on the same orbit, and points at it.
...
- **Neutral ships.** The traffic ship and the demo objects below belong to a fresh random account that no configuration knows. If there are no configured accounts, that account also owns the explorer.
- **Demo station.** A copy of the explorer's design 10 km along galactic +Z from the explorer, with the explorer's velocity. It is a beacon labelled `Demo station` with eight public bays numbered 0–7. Bay *n* is centred at (100 + 100*n*, 0, 0) m in host axes with identity rotation, a radius of 40 m and a mass capacity of 1e9 kg.
- **Gates.** `Demo gate 1` 20,000 km along +X from the explorer and `Demo gate 2` 10¹² m along +X. Both are beacons, paired with each other, enabled and public, with an aperture radius of 100 m and an exclusion radius of 1e7 m. Gates have zero velocity, no collision or spatial body, and are skipped by force application and integration.
...
The client separates transport ingestion, replication and interpolation into the [state modules](../crates/toy-sim-client/src/state). `SessionInfo` owns the world, generation, applied tick and sequence, capabilities, diagnostics and delivered command results. Network reception, buffered playback, outgoing commands and session metadata have separate resources. Only snapshot ingestion reads the buffered frames. It reconciles stable entities for contacts, controlled ships, views, screens and combat publications. Contact identity includes the information group, so observations from different groups remain distinct. ID-to-entity maps are derived lookup indexes.

Ship and contact entities hold pose samples and interpolated display components. Each view owns its camera, origin, exposure and sky state. `CameraOptions` holds focus and framing controls separately from the orbit data used to construct trajectories. Render instances relate to both their source observation and their view, allowing either lifetime to remove the associated visuals. Bevy asset handles own immutable downloaded assets; sky baking keeps its separate work budget. Egui panels query presentation components and enqueue actions through the outgoing resource.

Reception runs in `PreUpdate`, snapshot application in `FixedUpdate`, and input publication in `FixedPostUpdate`. `Update` then runs interpolation, celestial evaluation, view updates and rendering updates. World changes remove the old observations and reset selection. Missing observations remove their entities; a changed spatial lifetime creates fresh presentation samples. Docked ships retain private telemetry while their space pose is absent. MFD publications carry their instance revisions and screen definitions.
...
The [asset source](../crates/toy-sim-client/src/assets.rs) registers canonical `server://<hash>` paths before Bevy's asset plugin. Its reader forwards requests to the existing Tokio transport and returns verified bytes to typed loaders for ship designs, system definitions and universe catalogues. Decoding and compilation run through Bevy's asset pipeline. Ship observations and destruction publications hold design handles, system entities hold definition handles, and the universe browser holds its catalogue handle. Shared handles reuse loading and decoded assets; releasing the final handle allows unloading. A failed load stays failed until an explicit retry reloads its path. The asset status window exposes errors and retry actions. Individual entities appear as their assets become available.
...
`toy_sim_client::ui::run` ([ui.rs](../crates/toy-sim-client/src/ui.rs)) is the Bevy/egui app used by both `toy-sim-client` and `toy-sim-debug`. It renders the 3D scene, sky, orbit overlay, combat effects, instruments, MFDs and the universe browser from state frames and assets ([scene.rs](../crates/toy-sim-client/src/ui/scene.rs), [instruments.rs](../crates/toy-sim-client/src/ui/instruments.rs), [mfd.rs](../crates/toy-sim-client/src/ui/mfd.rs), [universe.rs](../crates/toy-sim-client/src/ui/universe.rs)). The debug build differs only in its playback depth and in the debug capabilities the server grants its account.

A UI synchronization system owns focus changes and subscriptions. Changing ships releases manual steering on the previous controlled ship, moves the instrument subscription and resets flight controls. Selection represents either a contact or a celestial body; camera focus requests use the same target type.

**On the first applied frame** the UI subscribes view 1 on the first group in the frame's track map (the account's default group, because the public group's all-ones ID sorts last), focused on the first controlled ship with a 1e8 m sphere, a limit of 256 and a work budget of 300,000. It also subscribes instruments for that ship and makes it the focused ship.

**"Ship controls" window.**

- Simulation time, playback depth and underruns, display frame rate, and the last status.
- **Info group key.** A masked field and "Join". The key must be exactly 64 hexadecimal digits.
- **"Open beacon map".** Subscribes view 8 on the public group, focused on the first ship, with `all = {Kind("beacon")}`.
- **Destination.** A field for three integer galactic micrometre coordinates, "Append waypoint", and "Offset from selected celestial / beacon" with "Rotate offset with reference body".
- For each controlled ship: a selectable ID that moves the focus and instrument subscription, presence, "Broadcast IFF", IFF owner, battery and shield, "Open view", "Resume", "Pause", "Undock", travel status and orders, "Travel to coordinates", and, for a selected beacon track, "Travel to selected beacon" and "Dock at selected station". For a selected non-public track, "Aim at selected contact". One "Open *title*" button per screen definition in the ship's presentation, which subscribes that slot at 10 Hz. "MFD slots" also opens slots 1–8 before the display has published definitions.
- The five most recent command errors.

**"Views" window.** A view radius slider from 10 m to 1e20 m controls the plots. For each view: its ID and completion, a "Select contact" drop-down, and a plot of its tracks projected onto the X–Z plane around the interpolated view origin.

**Flight windows.** "Flight computer" (status, "Hold attitude", "Manual / abort"), "Contacts" (select target, aim), "Navigation" (engage with throttle limit and stand-off, abort), "Weapons" (engage, hold fire, weapon rows) and "Ship hardware" (mass, health, execution timings, power, battery, heat, inventory, devices) act on the focused ship. The keyboard flight controls are listed in the [README](../README.md#client).

**Other windows.** One "Orbit · view *n*" window per view with the trajectory toggle and camera framing ([orbital-navigation.md](orbital-navigation.md)), "Sky and exposure", and "Universe", which loads the catalogue asset, searches systems and bodies, and for debug accounts can inspect a body or relocate the focused ship to it.

**MFD windows.** One window per screen update, titled with the screen title and ship. The screen widget forwards pointer moves, presses, releases and drags, key presses and releases, and text as `ScreenInput` ([mfds.md](mfds.md#over-the-network)). Closing the window unsubscribes.

**"Server debug" window.** Shown when the frame lists capabilities: pause, 1×, 10× and step; "Reset encounter"; for the focused ship "Recover focused ship", sensor range and occlusion, shield and hull heat injection; and "Server diagnostics" with the diagnostics listed above.
