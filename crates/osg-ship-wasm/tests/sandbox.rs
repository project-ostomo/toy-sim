...
fn c_and_no_std_rust_share_the_abi_with_persistent_isolated_memory() {
    for (bytes, destination) in [
        ...
        assert_eq!(
            ...
            osg_ships::DeviceSetting::Throttle(0.25)
        );
        assert_eq!(
...
fn invalid_record_lengths_return_status_but_actual_memory_access_traps() {
    let imports = r#"(import "ship_v31" "flight_read" (func $read (param i32 i32) (result i32)))"#;
    let body = r#"
...
const PATH_IMPORTS: &str = r#"
    (import "ship_v31" "spatial_path_put" (func $path (param i32 i32 i32 i32) (result i32)))
    (import "ship_v31" "spatial_marker_put" (func $marker (param i32 i32) (result i32)))
    (import "ship_v31" "snapshot_keep" (func $keep (param i64) (result i32)))
    (import "ship_v31" "snapshot_drop" (func $drop (param i64) (result i32)))
"#;
...
fn trapped_callback_discards_spatial_instrument_and_hardware_publications() {
    let imports = format!(
        r#"{PATH_IMPORTS}
        (import "ship_v31" "device_write" (func $write (param i64 i64 i32 i32) (result i32)))
        (import "ship_v31" "instrument_attitude_put" (func $attitude (param i32 i32) (result i32)))
    "#
    );
...
fn scan_reserves_full_cost_and_suspends_before_repeating_provider_work() {
    let imports =
        r#"(import "ship_v31" "sensor_scan" (func $scan (param i64 i32 i32 i32) (result i32)))"#;
...
fn fresh_requests_submitted_during_boot_are_delivered_after_startup() {
    let imports = r#"
        (import "ship_v31" "request_info" (func $info (param i32 i32 i32) (result i32)))
        (import "ship_v31" "request_reply" (func $reply (param i64 i64 i32 i32) (result i32)))
    "#;
    let body = r#"
...
fn unfinished_screen_frame_is_discarded_and_completed_frame_is_independent() {
    let imports = r#"
        (import "ship_v31" "screen_define" (func $define (param i32 i32) (result i32)))
        (import "ship_v31" "screen_begin" (func $begin (param i32 i32) (result i32)))
        (import "ship_v31" "screen_end" (func $end (param i64) (result i32)))
    "#;
...
fn borrowed_snapshot_expires_and_retained_snapshot_quota_is_explicit() {
    let imports = r#"
        (import "ship_v31" "tick_read" (func $tick (param i32 i32) (result i32)))
        (import "ship_v31" "snapshot_keep" (func $keep (param i64) (result i32)))
        (import "ship_v31" "snapshot_drop" (func $drop (param i64) (result i32)))
    "#;
    let body = r#"
...
fn suspended_callback_discards_expired_fire_and_publications_without_faulting() {
    let imports = format!(
        ...
        (import "ship_v31" "device_write" (func $write (param i64 i64 i32 i32) (result i32)))
        (import "ship_v31" "instrument_attitude_put" (func $attitude (param i32 i32) (result i32)))
        (import "ship_v31" "instrument_navigation_put" (func $navigation (param i32 i32) (result i32)))
        (import "ship_v31" "instrument_weapons_put" (func $weapons (param i32 i32 i32 i32) (result i32)))
        (import "ship_v31" "instrument_contacts_put" (func $contacts (param i32 i32) (result i32)))
        "#
    );
    let data = [
        record_data(
...
fn weapon_syscalls_reject_invalid_records_and_discard_staged_fire_on_fault() {
    ...
    for (setting, status, trap) in [
        ...
        let bytes = guest(
            r#"(import "ship_v31" "device_write" (func $write (param i64 i64 i32 i32) (result i32)))"#,
...
fn micropulse_engine_metadata_names_its_charge_resource_without_electrical_input() {
    use osg_ships::{Catalogue, EXAMPLE_CONTROLLER, starter};
    ...
    assert_eq!(spec.max_thrust_n, 100_000_000.);
    assert!((spec.max_thrust_n / spec.propellant_units_s - 50_000. * 9.80665).abs() < 1e-6);
}
...
fn durable_guest_data_survives_checkpoints_and_reboots() {
    let mut runtime = ControllerRuntime::new().unwrap();
    let bytes = guest(
        r#"(import "ship_v31" "persistent_read" (func $read (param i32 i32) (result i32)))
           (import "ship_v31" "persistent_write" (func $write (param i32 i32) (result i32)))"#,
...
fn failed_or_out_of_bounds_callbacks_do_not_commit_durable_writes() {
    for body in [
        ...
        let mut runtime = ControllerRuntime::new().unwrap();
        let bytes = guest(
            r#"(import "ship_v31" "persistent_write" (func $write (param i32 i32) (result i32)))"#,
            r#"(data (i32.const 64) "hello")"#,
            body,
        );
        let mut controller = ready(&mut runtime, &bytes);
...
#[test]
fn travel_read_rejects_bad_output_memory_before_calling_world_service() {
    ...
    impl ScanSource for CountingSource {
        ...
        fn query(
            &self,
            _: osg_model::ProgramQuery,
            _: bool,
            _: osg_model::wasm_world::ReplyCapacity,
        ) -> anyhow::Result<osg_model::ProgramReply> {
            self.0.fetch_add(1, Ordering::SeqCst);
            Ok(osg_model::ProgramReply::Pose(Default::default()))
        }
    }
