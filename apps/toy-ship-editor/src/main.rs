    ...
    pub fn validate(&mut self) {
        self.validation = self
            ...
            .and_then(|d| {
                ...
                Ok(format!(
                    "{} parts · {:.1} kg dry · {:.2} m³ storage · {:.0} hull",
                    d.parts.len(),
                    d.dry_mass,
                    d.capacity_m3,
                    d.hull
...
fn main() -> anyhow::Result<()> {
    ...
    App::new()
        ...
        .add_systems(
            Startup,
            (toy_sim_ship_view::prepare_visuals, viewport::setup),
        )
        .add_systems(
            Update,
            (viewport::visuals, toy_sim_ship_view::add_weapon_visuals),
        )
        .add_systems(EguiPrimaryContextPass, ui::editor)
        .add_systems(
            PostUpdate,
            viewport::camera.before(TransformSystems::Propagate),
        )
        .run();
    Ok(())
}
