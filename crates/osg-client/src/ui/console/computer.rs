...
pub(super) fn panel(ui: &mut egui::Ui, details: &ShipPresentation, display_ns: u64) {
    ...
    let cell_width = screen.width() / osg_model::serial::COLUMNS as f32;
    let cell_height = screen.height() / osg_model::serial::ROWS as f32;
    for (index, cell) in details
        .serial
        .cells
        .iter()
        .enumerate()
        .take(osg_model::serial::COLUMNS * osg_model::serial::ROWS)
    {
        let position = screen.min
            + egui::vec2(
                (index % osg_model::serial::COLUMNS) as f32 * cell_width,
                (index / osg_model::serial::COLUMNS) as f32 * cell_height,
            );
