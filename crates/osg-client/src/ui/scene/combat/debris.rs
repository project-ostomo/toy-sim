...
use osg_model::{CombatEvent, CombatEventKind, Pose};
...
impl Piece {
    ...
    fn cool(&mut self, age: f64) {
        ...
        while self.advanced + 1e-9 < age {
            ...
            self.temperature =
                osg_ships::thermal::cooled(self.temperature, self.capacity, self.area, dt);
            self.advanced += dt;
        }
    }
}
...
fn cool(
    ...
    for (entity, source, mut piece) in &mut pieces {
        ...
        if let Some(mut material) = materials.get_mut(&piece.material) {
            let rgb = osg_ship_view::thermal::blackbody(piece.temperature) * 0.8;
...
fn breakup(
    pose: &Pose,
    design: Option<&osg_ships::appearance::PreparedAppearance>,
    energy: f64,
    mass: f64,
    radius: f64,
    meshes: &mut Assets<Mesh>,
    ...
    let baseline = osg_ships::thermal::INITIAL_K;
        ...
        for (index, part) in design.parts.iter().take(MAX_FRAGMENTS - 8).enumerate() {
            ...
            pieces.push(piece(
                ...
                Vec3::from_array(
                    part.definition
                        .dimensions
                        .map(|dimension| dimension as f32 * osg_ships::GRID as f32),
                ),
    ...
    fn replay_preserves_fragment_physics_across_birth_and_expiry() {
        ...
        let event = CombatEvent {
            ...
            kind: CombatEventKind::Impact {
                normal: [0., 1., 0.],
                target: None,
                position: osg_model::GalacticPosition::ZERO,
                velocity_m_s: [0.; 3],
                energy_j: 1e6,
                shield: false,
            },
