...
use super::*;
use toy_sim_model::chat::{ChatMessage, MAX_MESSAGE_BYTES, valid_text};
        ...
        let output = area.show_rows(ui, row_height, state.layout.lines.len(), |ui, rows| {
            for index in rows {
                ...
                if let Some(message) = message {
                    response.on_hover_text(toy_sim_model::calendar::format_utc(
                        message.calendar_unix_ms,
                    ));
                }
            }
        });
...
fn heading(message: &ChatMessage, directory: &ownership::OwnershipDirectory) -> String {
    let timestamp = toy_sim_model::calendar::format_utc(message.calendar_unix_ms);
