...
pub fn validate_request(request: &toy_sim_model::routing::Request) -> Result<()> {
    ensure!(request.id != 0, "invalid route request id");
    ensure!(request.preferences.valid(), "invalid planning preference");
    ensure!(
        request.orders.len() <= toy_sim_model::routing::MAX_ORDERS,
...
mod tests {
    use super::*;
    use toy_sim_model::routing::{Plan, Request, Status};
