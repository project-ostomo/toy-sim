...
use toy_sim_model::travel::{FuelBudget, FuelRequirement, Presence};
use toy_sim_ships::{DeviceKind, Equipment};
...
pub(super) fn performance(world: &World, ship: Entity) -> Result<routing::ShipPerformance> {
    ...
    let forward =
        toy_sim_ships::orientation(design.blueprint.avionics.control_orientation) * DVec3::NEG_Z;
    let mut thrust = 0.0;
