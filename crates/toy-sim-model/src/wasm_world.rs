...
use toy_sim_ship_api::{abi::Text, world as abi};
...
pub fn query(query: &ProgramQuery) -> Result<ProgramReply, i32> {
    use abi::raw;
    use toy_sim_ship_api::abi::ERR_ARGUMENT;
...
pub fn command(action: ProgramAction) -> Result<(), i32> {
    ...
    let result = match action {
        ...
        ProgramAction::Block {
            ...
            if reason.len() > 256 {
                return Err(toy_sim_ship_api::abi::ERR_ARGUMENT);
            }
            unsafe {
...
mod tests {
    use super::*;
    use toy_sim_ship_api::abi::Record;
