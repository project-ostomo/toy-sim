use super::*;
use bevy::math::DQuat;
use osg_model::{IffIdentity, travel::*};
...
struct Environment {
    ...
    slip_enabled: bool,
    cancelled: bool,
    contacts: Vec<(osg_model::ContactRef, Pose, f64)>,
}
...
fn finite_contact_approach_retains_live_reference_and_rejects_unknown_contacts() {
    let reference = osg_model::ContactRef {
        group: id(100),
        track: id(101),
    };
    let env = Environment {
    ...
    if let Order::Guidance(guidance) = &mut input.orders[0] {
        guidance.target = Target::Contact(osg_model::ContactRef {
            group: id(999),
            ..reference
        });
    }
