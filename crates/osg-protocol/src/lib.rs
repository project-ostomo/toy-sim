...
mod tests {
    ...
    fn local_chat_roundtrips_and_requires_its_version_23_section() {
        let input = Message::Input(InputFrame {
            ...
            actions: vec![
                (
                    Id([2; 16]),
                    Action::ChatSubscribe(osg_model::chat::ChatSubscription {
                        revision: 1,
                        view: 1,
                    }),
                ),
    ...
    fn calendar_roundtrips_beyond_signed_nanosecond_range() {
        let mut frame = empty_frame();
        frame.calendar_unix_ms = osg_model::calendar::from_real_unix_ms(1_789_689_600_000);
        ...
        assert_eq!(
            osg_model::calendar::format_utc(decoded.calendar_unix_ms),
