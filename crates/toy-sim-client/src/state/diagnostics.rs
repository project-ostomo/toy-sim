...
pub(super) fn update(
    ...
    if frame_ms >= 250. && now - log.stall_at >= 1. {
        warn!(target: "toy_sim_client::diagnostics", frame_ms, tick = session.tick,
    ...
    if metrics.underruns > log.underruns && now - log.underrun_at >= 1. {
        warn!(target: "toy_sim_client::diagnostics", tick = session.tick,
    ...
    if metrics.catching_up != log.catching_up {
        info!(target: "toy_sim_client::diagnostics", catching_up = metrics.catching_up,
    ...
    if now - log.summary_at >= 5. {
        debug!(target: "toy_sim_client::diagnostics", fps = metrics.fps,
