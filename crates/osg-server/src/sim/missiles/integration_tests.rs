...
fn ordinary_same_owner_laser_can_destroy_a_missile() {
    ...
    weapon.0.command = Some(osg_ships::weapons::WeaponCommand {
        epoch_s: now,
...
fn shared_carrier_undock_checks_the_active_order_and_destroyed_hardware() {
    ...
    let active_undock = |revision| {
        travel::Travel(osg_model::travel::TravelState {
            autopilot_enabled: true,
            revision,
            orders: vec![osg_model::travel::Order::Undock.into()],
            status: osg_model::travel::Status::Active,
            ..Default::default()
        })
    };
    ...
    for (revision, order) in [(6, 0), (7, 1)] {
        assert!(
            travel::dispatch(
                world,
                parent,
                osg_model::ProgramAction::Undock { revision, order }
            )
            .is_err()
        );
        assert!(world.get::<Dormant>(parent).is_some());
    ...
    world
        ...
        .unwrap()
        .world_actions
        .push(osg_model::ProgramAction::Undock {
            revision: 7,
            order: 0,
        });
    ...
    assert!(matches!(
        ...
        osg_model::travel::Status::Blocked(reason) if reason == "ship has no active physical hardware"
    ));
}
