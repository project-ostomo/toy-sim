//! Bevy adapter: load an immutable binary star catalogue and query it off-thread.
use crate::{
    camera::MainCamera,
...
pub struct GaiaConfig {
    ...
    pub max_stars: usize,
    #[serde(default)]
    pub exclude_source_ids: Vec<u64>,
}
...
impl GaiaCatalogue {
    pub fn new(config: GaiaConfig) -> Self {
        ...
        let path = root.join("assets").join(&config.path);
        Self {
            config,
            path,
            reader: None,
    ...
    if field.reader.is_none() && field.opening.is_none() {
        ...
        field.opening = Some(
            AsyncComputeTaskPool::get()
                .spawn(async move { StarCatalogue::load(&path).map(Arc::new) }),
        );
    }
    ...
    if field.refresh_selection(origin, cutoff, completed) {
        if field.task.is_none() {
            if let Some(reader) = field.reader.clone() {
                let limit = field.config.max_stars.max(1);
                let exclude = field.config.exclude_source_ids.clone();
                field.task = Some(
                    AsyncComputeTaskPool::get()
                        .spawn(async move { select(&reader, origin, cutoff, limit, &exclude) }),
                );
...
/// Headless query benchmark, including nearest-star lookup.
pub fn benchmark(path: &std::path::Path) -> anyhow::Result<()> {
    let start = Instant::now();
    let reader = StarCatalogue::load(path)?;
    println!(
        "Loaded {} stars in {} buckets in {:.3} s",
        reader.len(),
        reader.bucket_count(),
    ...
    for magnitude in [6., 6.05, 9.] {
        for run in 0..3 {
            let s = select(
                ...
                min_brightness(magnitude),
                1_000_000,
                &[],
            )?;
            println!(
                "mag={magnitude} run={run} visible={} candidates={} buckets={} ms={:.3}",
                s.matched,
                s.candidates,
                s.buckets_searched,
                s.seconds * 1000.
