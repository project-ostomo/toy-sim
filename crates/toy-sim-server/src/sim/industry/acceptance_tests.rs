...
use toy_sim_ships::ShipBlueprint;
...
impl Fixture {
    ...
    fn spawn(world: &mut World, owner: Id, blueprint: ShipBlueprint, position: DVec3) -> Entity {
        ...
        let entity = vessel::spawn_ship(
            ...
            design,
            PreciseTransform {
                translation_um: toy_sim_model::GalacticPosition::from_meters(position),
                ..Default::default()
            },
            DVec3::ZERO,
    ...
    fn guest(&mut self, owner: Id) -> Entity {
        let guest = Self::spawn(
            &mut self.world,
            owner,
            toy_sim_ships::expedition_patrol(),
            DVec3::X * 200.0,
        );
...
#[test]
fn reserved_fuel_cannot_escape_through_transfer_refill_or_dock_services() {
    ...
    let recipe = fixture
        ...
        .find(|recipe| {
            recipe
                .inputs
                .iter()
                .any(|stack| stack.item == CargoItem::Resource("reactor_fuel".into()))
        })
        .unwrap()
        .clone();
    let fuel = fixture
        ...
        .resources
        .iter()
        .position(|resource| resource.id == "reactor_fuel")
        .unwrap();
    for entity in [fixture.facility, guest] {
        assert!(fixture.inventory(entity).tank_capacities_m3[fuel] > 0.0);
        fixture
            .world
            .get_mut::<hardware::ShipInventory>(entity)
            .unwrap()
    ...
    let physical_before = fixture.inventory(fixture.facility).cargo.clone();
    let job = fixture.start(&recipe);
    assert_eq!(fixture.inventory(fixture.facility).cargo, physical_before);
    let reserved = recipe
        .inputs
        .iter()
        .find(|stack| stack.item == CargoItem::Resource("reactor_fuel".into()))
        .unwrap()
        .clone();
    ...
    let recipient_before = fixture.inventory_bytes(guest);
    for command in [
        IndustryCommand::Transfer {
            source: facility_id,
            target: guest_id,
            item: reserved.item.clone(),
            quantity: 1,
        },
    ...
    fixture
        ...
        .insert(hardware::utilities::DockServices {
            cargo_kg_s: 10_000.0,
            power_w: 0.0,
        });
    fixture
    ...
    fixture
        ...
        .unwrap();
    assert_eq!(
        fixture.quantity(fixture.facility, &reserved.item),
        reserved.quantity
    );
    assert_eq!(fixture.quantity(guest, &reserved.item), 0);
    assert_eq!(
        fixture
    ...
    fixture
        ...
        .unwrap();
    assert!(
        fixture.quantity(guest, &reserved.item) > 0,
        "unreserved fuel should flow through the same service"
    );
}
...
fn remote_management_does_not_grant_material_transfer_or_private_inventory_access() {
    ...
    let remote = Fixture::spawn(
        &mut fixture.world,
        operator,
        toy_sim_ships::industry::starter_ship(),
        DVec3::X * 1e12,
    );
    ...
    fixture
        ...
        .unwrap()
        .translation_um = toy_sim_model::GalacticPosition::from_meters(DVec3::X * 200.0);
...
fn industry_grant_cannot_convert_an_organizations_materials_into_personal_ships() {
    ...
    .unwrap();
    assert!(
        toy_sim_ships::industry::item_volume_m3(&kit.item, &catalogue).unwrap()
            <= fixture
                .world
        ...
        .find(|recipe| {
            let volume = |items: &[ItemStack]| {
                items
                    .iter()
                    .map(|stack| {
                        toy_sim_ships::industry::item_volume_m3(&stack.item, &catalogue).unwrap()
...
fn blocked_ship_construction_retries_atomically_and_spawns_a_cold_mass_paid_hull() {
    ...
    let requirements =
        toy_sim_ships::industry::construction_requirements(&design, &catalogue).unwrap();
    let paid_mass =
        toy_sim_ships::industry::stack_mass_mg(&requirements.inputs, &catalogue).unwrap();
    assert_eq!(
        paid_mass,
        u128::from(toy_sim_ships::industry::mass_mg(design.dry_mass).unwrap())
    );
    assert!(
        design.dry_mass
            > design
...
fn physical_transfers_update_docked_mass_immediately_and_undocking_conserves_the_total() {
    ...
    let stored_before = fixture
        ...
        .unwrap()
        .0;
    let unit_mass = toy_sim_ships::industry::item_mass_kg(
        &item,
        &fixture.world.resource::<vessel::ShipCatalogue>().0,
    )
    .unwrap();
...
fn mine_loading_is_fractional_fair_authorized_and_never_accumulates_a_blocked_burst() {
    ...
    small_hold.capacity_m3 = toy_sim_ships::industry::item_volume_m3(&item, &catalogue).unwrap();
...
fn cold_shield_reserves_accept_only_paid_unreserved_coolant_with_exact_mass() {
    ...
    let design = fixture
        ...
        .0
        .clone();
    let cold = toy_sim_ships::ShipState::cold(&design, &catalogue);
    fixture
        .world
    ...
    let unit_mass = toy_sim_ships::industry::item_mass_kg(&coolant, &catalogue).unwrap();
    ...
    assert_eq!(
        fixture
            ...
            .0
            .shield_reserve_mg,
        toy_sim_ships::industry::mass_mg(capacity as f64 * unit_mass).unwrap()
    );
