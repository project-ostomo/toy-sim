...
mod firmware {
    use toy_sim_example_controller::{chatter::Chatter, firmware::Computer};
    ...
    extern "C" fn ship_display() {
        let _ = toy_sim_example_controller::firmware::draw_display();
    }
    ...
    extern "C" fn missile_tick(handle: u64) {
        if let Ok(observation) = sdk::missile() {
            if observation.handle == handle {
                let control = toy_sim_example_controller::missile::guide(&observation);
