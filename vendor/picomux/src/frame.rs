...
impl Frame {
    ...
    pub async fn read(mut rdr: impl AsyncRead + Unpin) -> std::io::Result<Self> {
        ...
        let header = Header {
            version: header_buf[0],
            command: header_buf[1],
            body_len: u16::from_le_bytes(header_buf[2..4].try_into().unwrap()),
            stream_id: u32::from_le_bytes(header_buf[4..8].try_into().unwrap()),
        };
        if header.version != 1 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "unsupported mux version",
            ));
        }
    ...
    pub fn bytes(&self) -> Bytes {
        ...
        buf[1] = self.header.command;
        buf[2..4].copy_from_slice(&self.header.body_len.to_le_bytes());
        buf[4..8].copy_from_slice(&self.header.stream_id.to_le_bytes());
