...
pub fn relocate(
    ...
    >,
    mut follow: MessageWriter<FollowTarget>,
    mut software: Query<&mut crate::vessel::ShipSoftware>,
) {
    ...
    for (entity, mut p, mut history, mut v, mut w, mut f, mut torque) in &mut ships {
        if let Ok(mut software) = software.get_mut(entity) {
            software.command(toy_sim_ship_wasm::Command::StopGuidance);
            software.command(toy_sim_ship_wasm::Command::Manual {
                throttle: 0.0,
                steering: [0.0; 3],
            });
        }
...
mod tests {
    use super::*;
    use crate::{
        orrery::{move_orrery, universe::Universe},
    ...
    fn activation_inspection_relocation_and_interpolation_are_ordered() {
        let configs = vec![
            crate::orrery::example_config(),
            crate::orrery::remote_test_config(),
        ];
        ...
        app.world_mut().resource_mut::<UniverseDebug>().inspect = Some("Remote I".into());
        app.update();
        ...
        app.world_mut().resource_mut::<UniverseDebug>().relocate = Some("Remote I".into());
        app.update();
