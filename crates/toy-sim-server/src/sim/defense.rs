...
use toy_sim_ships::{Equipment, utilities::UtilityDef};
...
fn witnessed_contact(world: &World, group: Entity, source: Entity) -> Option<ContactRef> {
    ...
    (fresh(track, tick(world))
        && !matches!(
            track.provenance,
            toy_sim_model::Provenance::Beacon | toy_sim_model::Provenance::Extrapolated
        ))
    .then(|| ContactRef {
...
struct ProtectedHull {
    position: toy_sim_model::GalacticPosition,
    velocity: DVec3,
    radius_m: f64,
}
...
fn incoming_missile(track: &Track, hull: &ProtectedHull) -> Option<f64> {
    if !track
        .tags
        .iter()
        .any(|tag| matches!(tag, toy_sim_model::Tag::Kind(kind) if kind == "missile"))
    {
        return None;
    }
