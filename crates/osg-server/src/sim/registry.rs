...
pub fn identity(name: &str) -> Id {
    let hash = blake3::derive_key("OpenSpaceGame celestial identity v1", name.as_bytes());
...
pub fn system_identity(name: &str) -> Id {
    let hash = blake3::derive_key("OpenSpaceGame system identity v1", name.as_bytes());
    let mut bytes: [u8; 16] = hash[..16].try_into().unwrap();
    bytes[6] = (bytes[6] & 0x0f) | 0x50;
    bytes[8] = (bytes[8] & 0x3f) | 0x80;
    Id(bytes)
...
pub fn gate_identity(local_catalogue: &str, remote_catalogue: &str) -> Id {
    let mut hash = blake3::Hasher::new_derive_key("OpenSpaceGame gate mouth identity v1");
...
fn prepare(universe: Arc<osg_universe::universe::Universe>) -> anyhow::Result<PreparedRegistry> {
    ...
    for (system_index, system) in universe.systems.iter().enumerate() {
        ...
        let asset = osg_universe::replication::SystemAsset {
            version: 1,
            system_id: system_id.0,
            body_ids: system
                .solver
                .iter()
                .map(|body| osg_universe::replication::BodyIdentity {
                    name: body.name.to_string(),
                    id: identity(&body.name).0,
                })
                .collect(),
            config: osg_universe::orrery_cfg::OrreryCfg {
...
impl UniverseRegistry {
    ...
    fn system_ref(&self, view: u64, index: usize) -> Option<CelestialSystemRef> {
        ...
        Some(CelestialSystemRef {
            ...
            system,
            definition,
            epoch_mjd_utc: osg_universe::replication::SIMULATION_EPOCH_MJD_UTC,
    ...
    pub fn system_refs(
        ...
        for view in views {
            ...
            if systems.len() > 32 {
                systems.truncate(32);
                view.completion = osg_model::Completion::ResultLimit;
            }
...
mod tests {
    use super::*;
    use osg_model::{Completion, GalacticPosition};

    #[test]
    fn inactive_remote_system_is_published_for_its_view_without_sensor_contacts() {
        let mut remote = osg_universe::example_config();
        ...
        world.insert_resource(
            Universe::from_configs(
                vec![osg_universe::example_config(), remote],
        ...
        let asset = osg_universe::replication::SystemAsset::decode(&bytes).unwrap();
    ...
    fn overlapping_systems_have_bounded_explicit_completion() {
        let template = osg_universe::example_config();
        ...
        let universe = osg_universe::universe::Universe::from_configs(
            configs,
            super::super::physics::GRAVITY_CUTOFF,
        )
        .unwrap();
