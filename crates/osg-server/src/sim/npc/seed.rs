...
use osg_ships::{Catalogue, CompiledShipDesign, ShipState};
use osg_universe::organizations::{self, OrganizationProfile, OrganizationRole};
...
pub(crate) fn population_id(organization: Id, role: &str) -> Id {
    let mut hash = blake3::Hasher::new_derive_key("OpenSpaceGame NPC population identity v1");
...
fn take_station_control(
    ...
    design.blueprint.firmware = osg_ships::Firmware::Custom(osg_ships::CHATTER_CONTROLLER.to_vec());
    ...
    {
        environment.firmware = Arc::from(osg_ships::CHATTER_CONTROLLER);
    }
    Ok(())
}
...
fn stock_facility(
    ...
    let stacks = if profile.roles.contains(&OrganizationRole::Industry) {
        ...
        .into_iter()
        .map(|(resource, quantity)| osg_model::industry::ItemStack {
