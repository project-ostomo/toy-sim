//! Closed-loop flight tests: real hardware, angular integrator and 10 Hz motion.
use super::*;
use bevy::math::DQuat;
use toy_sim_example_controller::hardware::{Actuation, Capability, Device, Hardware, Sample};
use toy_sim_example_controller::{Pilot, navigation::Phase};
...
struct Flight {
    ...
    target_hardware: Option<ShipState>,
    target_q: DQuat,
    target_w: DVec3,
}
impl Flight {
    ...
    fn tick(&mut self, commands: Vec<Command>) -> Step {
        let (mass, inertia) = self.hardware.mass_properties(&self.design, &self.cat);
        ...
        let measured = self.hardware.snapshot(&self.design);
        ...
        let sample = Sample {
            ...
            propellant_kg: self.hardware.inventory.quantities[0],
        };
        ...
        assert!(output.valid());
        self.hardware
            .apply_commands(&self.design, &output.devices)
            .unwrap();
        let engine = self.hardware.step(&self.design, &self.cat, 0.1);
        ...
        let target_thrust = if let Some(hardware) = &mut self.target_hardware {
            let e = hardware.step(&self.design, &self.cat, 0.1);
    ...
    fn engage(&mut self) {
        ...
        // First hardware tick powers the sensor, just as the real scenario does.
        self.tick(vec![]);
    ...
    fn forecast(&mut self) -> (DVec3, toy_sim_example_controller::prediction::Forecast) {
        for _ in 0..30 {
            self.tick(vec![]);
    ...
    fn finish(&mut self) {
        let range = self.pilot.navigation.r.length();
        let mut previous = self.pilot.navigation.r.dot(self.pilot.navigation.u);
        for _ in 0..10000 {
            self.tick(vec![]);
            let nav = &self.pilot.navigation;
            assert!(nav.phase.active(), "paused: {}", nav.reason);
            let closing = nav.r.dot(nav.u);
            if previous > 0.0 && closing <= 0.0 {
                assert!(nav.u.length() > 1.0);
                assert!(
                    nav.r.length() < (range * 0.1).max(1000.0),
                    "pass distance {}",
                    nav.r.length()
                );
                return;
        ...
        panic!("no pursuit pass; closest={}", self.min_separation);
    }
}
#[test]
...
#[test]
fn jousting_keeps_thrust_during_turns_and_builds_speed_to_the_pass() {
    let mut f = Flight::new(
        DVec3::ZERO,
        DVec3::X * 50_000.0,
        DVec3::ZERO,
    ...
    f.engage();
    assert!(f.pilot.navigation.throttle > 0.9);
    f.finish();
    assert!(f.pilot.navigation.u.length() > 500.0);
...
fn slow_attitude_control_and_throttle_limit_still_intercept() {
    ...
    f.hardware = ShipState::new(&f.design, &f.cat);
    f.hardware.test_loadout(&f.design, &f.cat);
    f.tick(vec![]);
    f.tick(vec![
        Command::SelectTarget(42),
        Command::EngageNavigation {
...
fn pursuit_tracks_target_maneuvers_without_reengagement() {
    for acceleration in [DVec3::X * 15., DVec3::new(8., 6., -3.)] {
        ...
        assert!(f.pilot.navigation.phase.active());
        assert!(f.pilot.navigation.u.length() > 100.0);
    }
}
...
fn intercepts_tumbling_target_at_ten_percent() {
    ...
    let mut target = ShipState::new(&f.design, &f.cat);
    target.test_loadout(&f.design, &f.cat);
    let commands = f
        .design
        .device_catalogue
        .iter()
        .filter_map(|d| {
            matches!(d.kind, DeviceKind::Engine { .. }).then_some(DeviceCommand {
                device: d.handle,
                setting: DeviceSetting::Throttle(crate::scenario::TRAFFIC_CHALLENGE_THROTTLE),
            })
        })
        .collect::<Vec<_>>();
    target.apply_commands(&f.design, &commands).unwrap();
    f.target_hardware = Some(target);
    f.target_w = crate::scenario::TRAFFIC_TUMBLE_BODY;
    f.engage();
    f.finish();
    assert!(f.pilot.navigation.u.length() > 100.0);
}
...
#[test]
fn jousting_forecast_reports_first_pass_and_continues_beyond_it() {
    let mut f = Flight::new(
        DVec3::ZERO,
        DVec3::X * 1000.0,
        DVec3::X * 1000.0,
...
#[test]
fn nearby_target_still_gets_full_thrust_without_station_keeping() {
    let mut f = Flight::new(
        DVec3::ZERO,
        DVec3::NEG_Z * 100.0,
        DVec3::ZERO,
    ...
    f.engage();
    let (_, path) = f.forecast();
    assert!(path.eta.unwrap() < 10.0);
    assert!(f.pilot.navigation.throttle > 0.9);
    assert_eq!(f.pilot.navigation.stand_off, 0.0);
}
...
fn complete_forecast_tracks_full_thrust_approach() {
    ...
    assert!(f.pilot.navigation.u.length() > 500.);
}
