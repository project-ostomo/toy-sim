...
pub(super) fn run_aero(app: &mut App) {
    app.add_systems(
        FixedUpdate,
        (update_aero_params, trivial_drag).run_if(in_state(GameState::Game)),
    );
}

fn update_aero_params(
    orrery: Res<Orrery>,
    mut obj: Query<(&PreciseTransform, &Velocity, &WithinSoi, &mut AeroParams)>,
...
fn trivial_drag(
    ...
    for (vel, ang_vel, mut force, mut torque, params) in objects.iter_mut() {
