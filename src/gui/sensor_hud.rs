...
pub fn overlay(
    ...
    observer: Single<&SensorContacts, With<ControlledVessel>>,
    objects: Query<(
        &GlobalTransform,
        &SpatialBody,
        Option<&Vessel>,
    ...
    for contact in &observer.visible {
        let Ok((transform, body, ship, celestial)) = objects.get(contact.entity) else {
            continue;
        };
        if (ship.is_some() && !hud.ships) || (celestial.is_some() && !hud.celestials) {
            continue;
        }
        let relative = transform.translation().as_dvec3() - camera_tf.translation().as_dvec3();
        let view_centre = DVec3::new(
            relative.dot(right.as_dvec3()),
            relative.dot(up.as_dvec3()),
            relative.dot(forward.as_dvec3()),
        );
        let Some((lower, upper)) = sphere_slope_bounds(view_centre, body.radius_m) else {
            continue;
        };
        ...
        let colour = if celestial.is_some() {
        ...
        if hud.labels {
            let name = ship
                ...
                .or_else(|| celestial.map(|c| c.0.as_str()))
                .unwrap_or("Object");
            let distance = if contact.distance_m < 1000.0 {
                format!("{:.0} m", contact.distance_m)
            } else {
                format!("{:.1} km", contact.distance_m / 1000.0)
            };
            let pos = egui::pos2(
