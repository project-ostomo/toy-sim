...
use osg_ships::{ShipBlueprint, missiles as missile_spec};
...
impl Fixture {
    ...
    fn spawn(
        ...
        assert_eq!(blueprint.controller_bytes(), osg_ships::EXAMPLE_CONTROLLER);
        let design = Arc::new(
            blueprint
    ...
    fn step(&mut self) {
        ...
        for (actor, previous) in [&self.station, &self.attacker].into_iter().zip(before) {
            ...
            assert!(software.last_gas_limit <= osg_ship_wasm::FUEL_PER_TICK);
