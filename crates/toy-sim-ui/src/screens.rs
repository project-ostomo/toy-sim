...
mod tests {
    ...
    fn render(
    ...
    ) -> (egui::Rect, Vec<ScreenEvent>) {
        let mut rect = egui::Rect::NOTHING;
        let mut input = vec![];
        let definition = ScreenDefinition {
            id: 2,
            title: Text64::new("Test"),
            width: 512,
            height: 256,
        };
        let mut output = ctx.run_ui(
            ...
            |ui| {
                ui.set_width(width);
                rect = egui::Rect::from_min_size(ui.cursor().min, egui::vec2(width, width / 2.));
                input = show(ui, &definition, None);
            },
        );
    ...
    #[test]
    fn custom_surface_maps_coordinates_focus_text_and_drag_release() {
        let ctx = egui::Context::default();
        let (rect, _) = render(&ctx, 256., vec![]);
        let p = rect.center();
        render(&ctx, 256., vec![egui::Event::PointerMoved(p)]);
        let (_, events) = render(&ctx, 256., vec![button(p, true)]);
        let press = events.iter().find(|e| e.kind == 1).unwrap();
        assert!((press.x - 256.).abs() < 1. && (press.y - 128.).abs() < 1.);
        render(&ctx, 256., vec![button(p, false)]);
        let (_, events) = render(
            &ctx,
            256.,
            vec![
                ...
                egui::Event::Text("é".repeat(40)),
            ],
        );
        assert!(
            events
                .iter()
                .any(|e| e.kind == 3 && e.code == 'R' as u64 && e.modifiers == 1)
        );
        let text: String = events
            ...
            .collect();
        assert_eq!(text, "é".repeat(40));
        assert!(events.iter().all(|event| event.text.as_str().is_some()));
        render(&ctx, 256., vec![button(p, true)]);
        let outside = rect.right_bottom() + egui::vec2(80., 80.);
        render(&ctx, 256., vec![egui::Event::PointerMoved(outside)]);
        let (_, events) = render(&ctx, 256., vec![button(outside, false)]);
        assert!(events.iter().any(|e| e.kind == 2 && e.x > 512.));
        let (large, _) = render(&ctx, 512., vec![]);
        let (_, events) = render(&ctx, 512., vec![egui::Event::PointerMoved(large.center())]);
        assert!(
            events
                .iter()
                .any(|e| e.kind == 0 && (e.x - 256.).abs() < 1.)
        );
    }
}
