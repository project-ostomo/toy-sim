        ...
        let slip_power: f64 =
            d.0.parts
                ...
                .filter_map(|part| match part.definition.equipment {
                    Equipment::Utility {
                        utility: osg_ships::utilities::UtilityDef::SlipDrive { power_w },
                    } => Some(power_w),
                    _ => None,
                })
                .sum();
...
pub fn resources(
    ...
    energy: u64,
) -> osg_ship_api::abi::ShipResources {
    osg_ship_api::abi::ShipResources {
        hull_hp: hull,
        hull_max_hp: d.hull,
        ...
        .for_each(|(d, mut h, mut outputs, absent)| {
            ...
            let fitted_sensor = d.0.parts.iter().any(|part| {
                matches!(
                    part.definition.equipment,
                    Equipment::Utility {
                        utility: osg_ships::utilities::UtilityDef::Sensor { .. }
                    }
                )
            });
            if !absent
    ...
    for (entity, delta) in changes {
        ...
        while visited.len() < identities.0.len() {
            ...
            let host_id = {
                ...
                match &presence.0 {
                    osg_model::travel::Presence::Docked { host, .. }
                    | osg_model::travel::Presence::StoredInWreck(host) => *host,
                    _ => break,
                }
            };
...
pub fn shutdown(world: &mut World, ship: Entity) {
    ...
    if let Some(mut t) = world.get_mut::<ShipThermal>(ship) {
        ...
        t.0.shield_state = osg_ship_api::abi::SHIELD_OFF;
    }
}
...
fn dormant_thermal(
    ...
    ships
        ...
        .for_each(|(design, mut hull, mut thermal, mut elapsed, presence)| {
            if presence.is_some_and(|p| matches!(p.0, osg_model::travel::Presence::Docked { .. })) {
                return;
            }
