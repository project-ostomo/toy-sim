...
impl CatalogueTree {
    ...
    pub fn resolved(&self, origin: GalacticPosition, min_radius_over_distance: f64) -> Vec<usize> {
        let threshold = min_radius_over_distance.powi(2);
        self.walk(
            |n| n.radius.powi(2) < threshold * box_distance_squared(n, origin),
            |e| e.radius.powi(2) >= threshold * e.position.relative_to(origin).length_squared(),
        )
    }
    ...
    pub fn nearest_unresolved(&self, origin: GalacticPosition, excluded: &[usize]) -> f64 {
        if self.nodes.is_empty() {
            return f64::INFINITY;
        }
...
mod tests {
    ...
    fn tree_queries_match_brute_force() {
        ...
        let excluded = tree.resolved(origin, 0.02);
        let mut resolved = excluded.clone();
        resolved.sort_unstable();
        let brute_resolved: Vec<_> = tree
            .entries
            .iter()
            .enumerate()
            .filter(|(_, e)| {
                e.radius.powi(2)
                    >= 0.02_f64.powi(2) * e.position.relative_to(origin).length_squared()
            })
            .map(|(i, _)| i)
            .collect();
        assert_eq!(resolved, brute_resolved);
        let near = tree.nearest_unresolved(origin, &excluded);
        let brute = tree
            .entries
            .iter()
            .enumerate()
